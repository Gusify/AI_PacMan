...
class Pacman(Entity):
    ...
    def update(self, walls: List[pygame.Rect], pellets: List[Pellet], ghosts: List['Ghost'], dt: float) -> None:
        ...
        if danger and self.mode != "flee":
            self.mode = "flee"
        elif not danger and self.mode == "flee" and self.flee_cooldown == 0.0:
            self.mode = "seek"
            # Set a short cooldown to prevent Pac-Man from instantly re-entering danger
            self.flee_cooldown = 0.5 

        new_direction = Vector2(0, 0)

        # 3. Apply Mode-Specific Movement Logic
        if self.mode == "seek":
            ...
            if nearest_pellet:
                ...
                if path:
                    new_direction = self._path_to_direction(start_pos, path[0])

        elif self.mode == "flee":
            new_direction = self.get_escape_direction(danger_ghosts, walls)

        # 4. Apply new direction and move
        self.direction = new_direction
        if not self.move(walls):
            self.direction.update(0, 0)

    def draw(self, surface: pygame.Surface) -> None:
        # Show the mode by changing the color slightly
        color = (255, 128, 0) if self.mode == "flee" else self.color 
        pygame.draw.circle(surface, color, self.rect.center, TILE_SIZE // 2 - 1)


class Ghost(Entity):
    # ... (Ghost class is unchanged) ...
    def __init__(
        self,
        start_pos: Vector2,
...
class Game:
    # ... (Game class is mostly unchanged, but update needs a modification) ...
    ...
    def reset_entities(self) -> None:
        self.pacman.reset()
        # Ensure Pacman's mode is reset
        self.pacman.mode = "seek"
        self.pacman.flee_cooldown = 0.0
        ...
        for event in pygame.event.get():
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key in DIRECTION_VECTORS:
                    # Input is still recorded, but the AI overrides it.
    ...
    def update(self, dt: float) -> None:
        ...
        # MODIFIED: Pass ghosts and delta time (dt) to pacman.update
    ...
    # ... (rest of Game class is unchanged) ...
        ...
        for pellet in self.pellets[:]:
            if pellet.collides(self.pacman.rect):
                ...
                if pellet.power:
                    ...
                    # When a power pellet is eaten, Pacman's mode must be SEEK
                    self.pacman.mode = "seek"
                    self.pacman.flee_cooldown = 0.0
                else:
                    self.score += 10
    ...
    def _draw_hud(self) -> None:
        ...
        # Display current AI mode for debugging/feedback
        mode_text = f"Mode: {self.pacman.mode.upper()}"
        if self.pacman.mode == "flee":
            mode_text += f" (CD: {self.pacman.flee_cooldown:0.1f}s)"
        mode_surface = self.font.render(mode_text, True, YELLOW)
        ...
        self.screen.blit(mode_surface, (10, self.maze.pixel_height + 30)) # Display mode

        if self.power_timer > 0.0:
            timer_surface = self.font.render(f"Power: {self.power_timer:0.1f}s", True, POWER_COLOR)
            self.screen.blit(timer_surface, (self.maze.pixel_width // 2 - timer_surface.get_width() // 2, self.maze.pixel_height + 10))
